...
def get_token():
    ...
    if not os.path.exists(os.path.join(os.getcwd(),'token.key')):
        with open(os.path.join(os.getcwd(),'token.key'),'w') as f:
            f.write('token')
    # 读取token.key文件中的token值
    with open(os.path.join(os.getcwd(),'token.key'),'r') as f:
        token=f.read()
    return token
